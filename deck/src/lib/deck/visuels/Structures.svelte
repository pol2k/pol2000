...
ages &lt;- c(24, 30)</pre>
  ...
  <div class="bas">
    {#key e}
      <pre class="console">{#each TEMPS[e].lignes as l}<span class="in"><span class="prompt">&gt;</span> {l.in}</span>
<span class="out">{l.out}</span>{/each}</pre>
      <div class="legende">
...
<style>
  ...
  .nom { font-family: var(--dk-mono); font-weight: 600; font-size: 1.1em; width: 3.2em; text-align: right; transition: color 0.3s; }
  ...
  .zone { position: relative; display: flex; align-items: flex-start; gap: 0.7em; padding-bottom: 0.2em; }
  .nom.df { align-self: center; }
  .grille { display: grid; grid-template-columns: 1.8em 6.2em 4.4em; grid-auto-rows: 2.3em; border-top: 3px solid var(--dk-encre); border-left: 3px solid var(--dk-encre); font-family: var(--dk-mono); font-size: 1.15em; }
  .grille span { border-right: 3px solid var(--dk-encre); border-bottom: 3px solid var(--dk-encre); display: flex; align-items: center; justify-content: center; font-weight: 600; transition: opacity 0.4s; }
  ...
  .colonne { position: absolute; left: calc(3.2em + 0.7em + 3px + (1.8em + 6.2em) * 1.15); top: 0; width: calc(4.4em * 1.15); display: grid; grid-auto-rows: calc(2.3em * 1.15); font-family: var(--dk-mono); font-size: 1.15em; opacity: 0; pointer-events: none; transition: transform 0.7s ease, opacity 0.5s; }
  .colonne span { border: 3px solid var(--dk-accent); margin-bottom: -3px; display: flex; align-items: center; justify-content: center; font-weight: 600; color: var(--dk-accent); background: var(--dk-fond); }
  ...
  .dollar { position: absolute; left: calc(3.2em + 0.7em + 3px + (1.8em + 6.2em + 4.4em) * 1.15 + 0.6em); top: 1.1em; font-family: var(--dk-mono); font-weight: 600; font-size: 0.85em; color: var(--dk-accent); opacity: 0; transition: opacity 0.4s 0.4s; white-space: nowrap; }
  .dollar.vu { opacity: 1; }
  .moy { position: absolute; left: calc(3.2em + 0.7em + 3px + (1.8em + 6.2em + 4.4em) * 1.15 + 6.9em + 4.4em * 1.15); top: 2.6em; font-family: var(--dk-mono); font-weight: 600; font-size: 1.3em; color: var(--dk-accent); opacity: 0; transition: opacity 0.4s; white-space: nowrap; }
  .moy.vu { opacity: 1; }
