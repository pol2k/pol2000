  ...
  <div class="grille">
    {#each F as f, i}
      <div class="tuile" style="animation-delay: {i * 110}ms">
        ...
        <p>{f.quoi}</p>
        <div class="cmd"><code>{f.code}</code><span class="btn"><Copier texte={f.code} /></span></div>
...
<style>
  ...
  .cmd { position: relative; background: var(--dk-fond-2); border: 2px solid var(--dk-encre); border-left-width: 0.34em; border-left-color: var(--dk-accent); padding: 0.45em 4.4em 0.45em 0.8em; }
  .cmd code { background: none; padding: 0; font-size: 0.86em; }
  .btn { position: absolute; top: 0.35em; right: 0.35em; line-height: 1; }
