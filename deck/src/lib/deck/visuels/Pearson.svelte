<script>
   ...
   * étudie, ou la façon dont on l'étudie ? La méthode. Quatre temps.
   *
   ...
   *   3  La citation de Karl Pearson (1892), citée par King, Keohane et
   *      Verba (1994), Designing Social Inquiry, p. 9 (traduction libre),
   *      puis la ligne rouge : le sujet ne décide jamais.
   */
...
<div class="visuel pe-fig" bind:this={hote}>
  <div class="pe-scene" class:pe-recule={e >= 3}>
    <svg
    ...
    >
      ...
      {#each CENTRES as cx, i}
        ...
        <g class="pe-tuile" style="--k: {i}">
          ...
          {:else if i === 3}
            <!-- Une guerre : deux flèches opposées. -->
            <path d="M {cx - 38} {cy - 13} L {cx - 4} {cy - 13} M {cx - 14} {cy - 21} L {cx - 4} {cy - 13} L {cx - 14} {cy - 5}" class="pe-trait" />
            <path d="M {cx + 38} {cy + 15} L {cx + 4} {cy + 15} M {cx + 14} {cy + 7} L {cx + 4} {cy + 15} L {cx + 14} {cy + 23}" class="pe-trait" />
          {:else}
  ...
  <div class="pe-bloc" class:pe-vu={e >= 3}>
    <p class="pe-cit">«&#8239;L’unité de toute science réside dans sa seule méthode, non dans sa matière.&#8239;»</p>
    <p class="pe-attr">Karl Pearson (1892), cité par King, Keohane et Verba (1994), <i>Designing Social Inquiry</i>, p. 9 (traduction libre)</p>
    <p class="pe-chute">Le sujet ne décide jamais.</p>
  </div>
</div>

<style>
  .pe-fig { display: flex; flex-direction: column; gap: 0.5em; }
  .pe-scene { transition: opacity 0.5s; }
  .pe-scene.pe-recule { opacity: 0.32; }
  svg { width: 100%; height: auto; max-height: 44vh; display: block; overflow: visible; }
  ...
  .pe-bloc.pe-vu { opacity: 1; border-top-color: var(--dk-encre); }
  ...
  .pe-bloc.pe-vu .pe-cit { transform: none; }

  .pe-attr { margin: 0; font-size: 0.68em; line-height: 1.35; color: var(--dk-gris); }
  .pe-attr i { font-style: italic; }
  ...
  .pe-bloc.pe-vu .pe-chute { opacity: 1; }

  @keyframes pe-pose { from { opacity: 0; transform: translateY(-26px); } to { opacity: 1; transform: none; } }

  @media (prefers-reduced-motion: reduce) {
    .pe-tuile { animation: none; }
    .pe-scene, .pe-chemin, .pe-boite, .pe-boite-c, .pe-trait-g, .pe-bloc, .pe-cit, .pe-chute { transition: none; }
  }
</style>
