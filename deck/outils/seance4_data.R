...
CONSOLES <- list(
  # La convention du cours : df_raw ne change jamais; df_clean part vide et
  # se remplit une colonne à la fois. Jamais count() : table() avant, table() après.
  # Le codebook est dans R : la question et ce que chaque code veut dire.
  codebook = c('attr(df_raw$cps25_demsat, "label")', "table(df_raw$cps25_demsat)"),
  # La base propre commence vide, une ligne par répondant.e.
  vide = c("df_clean <- data.frame(id = 1:nrow(df_raw))", "dim(df_clean)"),
  # La scolarité en trois catégories.
  scolarite = c("table(df_raw$cps25_education)",
  ...
  satisfaction = c("table(df_raw$cps25_demsat)",
  ...
  # Oui, non, et « ne sait pas ».
  ne_canada = c("table(df_raw$cps25_bornin_canada)",
  ...
  na = c("df_clean$gauche_droite <- na_if(df_raw$cps25_lr_scale_bef_1, -99)",
         "mean(df_clean$gauche_droite, na.rm = TRUE)",
         "sum(is.na(df_clean$gauche_droite))"),
  nonpose = c("sum(is.na(df_raw$cps25_votechoice))", "table(df_raw$cps25_v_likely)"),
  # La base propre, complétée et sauvegardée.
  propre = c("df_clean$age <- as.numeric(df_raw$cps25_age_in_years)
...
df_clean <- get("df_clean", envir = globalenv())
etapes <- c("age", "ses_education", "gauche_droite", "satisfaction", "ne_canada", "vote")
nettoyees <- df_clean[, etapes]  # les colonnes nettoyées, sans id
compte <- function(x, niveaux) as.integer(table(factor(as.numeric(x), levels = niveaux)))
lr <- as.numeric(df_raw$cps25_lr_scale_bef_1)
...
choix <- unique(c(1:3, which(lr == -99)[1], which(as.numeric(df_raw$cps25_demsat) == 5)[1],
                  which(is.na(df_raw$cps25_votechoice))[1]))
brut <- df_raw[choix, c("cps25_age_in_years", "cps25_education", "cps25_lr_scale_bef_1", "cps25_demsat", "cps25_bornin_canada", "cps25_votechoice")] |>
  mutate(across(everything(), as.numeric))
propre <- nettoyees[choix, ] |> mutate(vote = as.character(vote))

# Ce que deviennent les réponses : pertes à chaque colonne ajoutée.
restants <- sapply(seq_along(etapes), function(k) nrow(drop_na(nettoyees[, etapes[1:k], drop = FALSE])))
...
out <- c(
  ...
  "",
  "/* Les dimensions de df_raw et de df_clean (avec sa colonne id). */",
  sprintf("export const DIMENSIONS = %s;", J(list(brut = dim(df_raw), propre = dim(df_clean)))),
  ...
  sprintf("export const SCOLARITE = %s;", J(list(
    codes = 1:12, etiquettes = unname(names(attr(df_raw$cps25_education, "labels"))),
    effectifs = compte(df_raw$cps25_education, 1:12),
    groupes = { t <- table(df_clean$ses_education, useNA = "ifany"); as.list(setNames(as.integer(t), ifelse(is.na(names(t)), "NA", names(t)))) }))),
  "/* La satisfaction envers la démocratie (cps25_demsat), codes 1 à 5. */",
  sprintf("export const SATISFACTION = %s;", J(list(
    codes = 1:5, etiquettes = unname(names(attr(df_raw$cps25_demsat, "labels"))),
    effectifs = compte(df_raw$cps25_demsat, 1:5)))),
  "/* Né au Canada (cps25_bornin_canada), codes 1 à 3. */",
  sprintf("export const NE_CANADA = %s;", J(list(effectifs = compte(df_raw$cps25_bornin_canada, 1:3)))),
  "/* L'intention de vote : les « ne sait pas » (code 7) et les NA, selon cps25_v_likely. */",
  sprintf("export const VOTE_VIDE = %s;", J(list(
    n = nrow(df_raw), nsp = sum(as.numeric(df_raw$cps25_votechoice) == 7, na.rm = TRUE), na = sum(is.na(df_raw$cps25_votechoice)),
    raisons = as.list(table(as.numeric(df_raw$cps25_v_likely)[is.na(df_raw$cps25_votechoice)]))))),
  ...
  sprintf("export const PERTES = %s;", J(list(etapes = etapes, restants = restants, n = nrow(nettoyees)))),
  "",
